...
- **Изолированная песочница WASM**: Выполняет пользовательский код в безопасной виртуальной машине на базе чистого Go-рантайма `wazero` (без CGO, из коробки работает в `scratch` docker-образах).
...
- **Отказоустойчивость и утечки ресурсов**: Исключает утечки благодаря переиспользованию HTTP-клиента с поддержкой контекстов (`context.Context`) и явному высвобождению ресурсов рантайма wazero.
